...
**Fix (recommended by plan, NOT applied)** — the app already does uniform scaling via the CSS `transform: scale(var(--scale))` in `App.css`, which handles any aspect ratio gracefully. Drop the Rust handler. If you want to keep the lock, move the state into `AppState` and add a re-entrancy flag so self-induced resizes are ignored.

**Status:** `lib.rs` contains a `Resized` event handler that enforces a 2:1 aspect ratio via `set_size`. This handler was **accidentally removed during duplicate WindowEvent handler cleanup**, causing window resizing to drift away from 2:1. It was **restored immediately** (commit `2d8e11d`) and is functioning correctly. The plan recommends deleting this handler; it remains because the user prefers CSS-only scaling with the lock for visual consistency.
...
| **D-5** | **Keep the 2:1 aspect-ratio lock?** ([M-12](#m-12--aspect-ratio-enforcement-fights-the-user-via-process-global-statics)) | **Keep it.** The CSS `transform: scale()` handles arbitrary sizes, but the Rust lock provides visual consistency. Dropped accidentally during a cleanup and restored. |
